// Simulation start/stop/reset and speed controls
import { WasmExports } from '../../types.js';
import { Grid3DRenderer } from '../../renderer/Grid3DRenderer.js';

export function createSimulationControls(container: HTMLElement, wasm: WasmExports, renderer: Grid3DRenderer) {
    ...
    const createMetricElement = (label: string): HTMLElement => {
    ...
    (window as any).updateSimulationMetrics = updateMetrics;
    ...
    function simulationLoop(now: number) {
        if (!running) return;
        ...
        (window as any).algoAnimationFrameId = requestAnimationFrame(simulationLoop);
    }
